...
def parse_dict(s, inden):
    ...
    for i in range(len(keys)):
        ...
        if item[0] == '[': # 배열인 경우
            ret[k] = parse_arr(item)
        elif item[0] == '<': # 사전인 경우
            ret[k] = parse_dict(item, inden + 4)
        else:
...
for i in range(len(objs)):
    objs[i][1] = parse_dict(objs[i][1], 4)
    print(objs[i][0], ':', objs[i][1])


# get tree
G = [Vertex(1) for _ in range(max_id + 1)] # tree structure을 저장


def make_vertex(data, ign=False):
    v = None
    if isinstance(data, str):
        ...
        v = Vertex(4)
        for i in data:
            v.add_child(make_vertex(i))
    elif isinstance(data, dict):
        v = Vertex(3)
        for k in data:
            v.add_child(make_vertex(data[k]), k)
    global G
    G.append(v)
    return len(G) - 1
...
for obj in objs:
    for k in obj[1]:
        if k == '/Parent': # 오브젝트를 가리키지 않게 처리
            G[obj[0]].add_child(make_vertex(obj[1][k], True), k)
        else:
            G[obj[0]].add_child(make_vertex(obj[1][k]), k)
